...
ARM은 양 장면에서 raw mask보다 후보를 늘렸고, 3×3보다는 제한된 범위로 확장했다.
...
0.080916%였다. `0.25±0.003` threshold 경계 밖 mismatch는 각각 0.007426%,
0.000563%였다. 따라서 bit-identical이라고 표현하지 않으며 R8/linear filtering과
threshold 경계 오차를 수치로 보존한다.
...
즉, ARM 확장이 일부 Candidate-Jitter case에서 작은 reference 이득을 보였지만 두 장면과
두 profile에 걸친 일관된 품질 개선은 확인되지 않았다.
...
3. 3×3보다 후보 확장 범위는 작지만 4-pass 비용은 오히려 약 3배 높다.
4. Candidate-Jitter의 일부 reference 지표는 소폭 개선됐지만 document profile을 포함한
   일관된 품질 이득은 없다.
5. 따라서 현재 full→half→quarter→half→full 구현은 최종 개선안으로 채택하지 않고
   600×3 성능 및 CGVQM formal 확대를 중단한다.
6. 구현과 부정적 결과는 ablation 근거로 보존한다. 후속으로 재검토한다면 pass fusion,
