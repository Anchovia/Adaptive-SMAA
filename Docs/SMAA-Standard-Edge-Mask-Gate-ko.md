...
전체 480-frame 경로의 두 장면 비교를 완료했다. 원본 Standard control 960 frame의
hash가 기존 정식 결과와 전부 일치했고, 공식 CGVQM-2의 중앙 이동과 이동→정지 네 비교에서
모두 원본이 우세했다. 특히 edge-mask는 정지 후에도 두 출력이 2-frame 주기로 교대했다.
앞의 성능 결과와 함께 현재 구현을 개선안으로 채택하지 않고 대조 실험으로 보존한다.
수치·조건·재현 도구는 [후속 품질 보고서](Standard-Edge-Mask-Quality-20260917/report.md)에 기록했다.
