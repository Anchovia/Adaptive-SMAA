param(
    ...
    [string]$Manifest='tmp/recovered-clipping/runs.json'
)
    ...
    foreach($scene in @('bistro','minecraft')) {
        ...
        foreach($pair in $pairs) {
            ...
            foreach($variant in $variants) {
                ...
                $signed=$variant-band 1;$domain=$variant-shr 1
                $changed=$source.Replace('#define SMAA_RECOVERED_SIGNED_CHROMA 0',"#define SMAA_RECOVERED_SIGNED_CHROMA $signed").Replace('#define SMAA_RECOVERED_YCOCG_CLAMP 0',"#define SMAA_RECOVERED_YCOCG_CLAMP $domain")
                ...
                $record=[ordered]@{label=$label;scene=$scene;mode=$Mode;variant=$names[$variant];signed_chroma=$signed;ycocg_clamp=$domain;pair=$pair;
