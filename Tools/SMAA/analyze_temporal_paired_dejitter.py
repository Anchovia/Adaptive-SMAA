"""Paired current/history de-jitter gate: controls, changed masks, reconstruction and timing."""
import argparse
import csv
import hashlib
import json
...
NATIVE = 'O-T2X-R'
POINT = 'ABL-HistoryLinear-R'
LINEAR = 'ABL-PairedDeJitter-R'
SCALAR = 'ABL-ScalarWeight-001-R'
SLINEAR = 'ABL-ScalarPairedDeJitter-001-R'
...
def read_receipt(path, scene, phase):
    receipts = [r for r in json.loads(path.read_text(encoding='utf-8-sig')) if r['phase'] != 'StaticCapture']
    ...
    for token in ('Aggregate: PASS', f'Scene: {scene}', '1920 x 1061',
                  'Paired de-jitter gate: Linear current UV+j', 'API:  DirectX11'):
...
def quality(a):
    ...
    pairs = [(capture/NATIVE, old/NATIVE), (capture/SCALAR, scalar_prior/SCALAR),
             ...
             (capture/POINT, bench/('20260921_152300' if a.scene == 'bistro' else '20260921_151748')/POINT),
    ...
    dejitter_prior = bench/('20260921_155746' if a.scene == 'bistro' else '20260921_155323')
    for m in (DMASK, DBASE):
        for f in expected:
            assert sha(capture/m/f) == sha(dejitter_prior/m/f)
    static_gate = json.loads((bench.parents[2]/f'Docs/Temporal-Paired-DeJitter/{a.scene}-static.json').read_text())
    assert static_gate['static_gate'] == 'PASS'
    static_capture = Path(static_gate['receipt']['report']).parent
    for i in range(40):
        f = expected[i]
        assert sha(capture/LINEAR/f) == sha(static_capture/LINEAR/f)
    rows, counts, prev, hashes = [], [], {}, {m: [] for m in MODES}
    streams = {m: hashlib.sha256() for m in MODES}
    ...
    result = dict(scene=a.scene, classification='Paired current/history de-jitter engineering gate; changed selector and current alpha, Linear corrected history; unchanged raw spatial history storage; spatial proxy, not temporal ground truth',
        ...
        selection_semantics_frames=480, first_frame_differing_channels=first_seed_differences, corrected_input_hash_comparisons=480, full_static_hash_bridge=40,
    ...
    labels = ('Original T2X-R', 'Full paired de-jitter', 'Scalar original', 'Scalar paired de-jitter')
...
def performance(a):
    ...
    for r, d in zip(rows, distributions):
        assert r['threshold'] == (.01 if r['mode'] in (SCALAR, SLINEAR) else 0)
    ...
    comparisons = {}
    for candidate, control in ((POINT, NATIVE), (LINEAR, NATIVE), (LINEAR, POINT), (SLINEAR, LINEAR), (SLINEAR, POINT),
