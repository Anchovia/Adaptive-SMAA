...
def make_four_mode_gif(
    ...
    name = f"candidate_ablation_{roi_name}_{start:05d}_{end - 1:05d}.gif"
    frames[0].save(
        output / name,
        save_all=True,
...
def make_four_mode_sheet(
    ...
    name = (
        f"candidate_ablation_sheet_{roi_name}_"
...
def main() -> None:
    ...
    output = (
        ...
        else root / "CandidateOnlyAnalysis"
    )
    output.mkdir(parents=True, exist_ok=True)

    paths, resolution, validation = validate_inputs(root, args.expected_frames)
    ...
    csv_name = "candidate_only_ablation_metrics.csv"
    ...
    json_name = "candidate_only_ablation_summary.json"
    (output / json_name).write_text(
        json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8"
    )

    report = [
        "# SMAA Candidate-only 구성요소 ablation 분석",
        "",
        "## 범위",
        "",
    ...
    report.extend(f"- 4-way 비교 GIF: `{name}`" for name in gifs)
    report.extend(f"- 4-way sequence sheet: `{name}`" for name in sheets)
    report.append("")

    report_name = "SMAA-Candidate-Only-Ablation-Analysis-ko.md"
    (output / report_name).write_text("\n".join(report), encoding="utf-8")
    print(f"Candidate-only ablation analysis complete: {output}", flush=True)
