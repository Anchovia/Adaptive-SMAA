param(
    [ValidateSet('StaticCapture','Capture','Smoke','Benchmark')][string] $Phase = 'Capture',
    [ValidateSet('bistro','minecraft')][string] $Scene = 'bistro',
    [string] $Receipt = 'tmp/temporal-paired-dejitter-runs.json'
)
...
$argsForDemo = @("-smaaTemporalPairedDeJitter$Phase",$Scene)
