...
OUT=ROOT/'Docs/Recovered-Sharpen-Segment-20260915'
...
def main():
    jobs=json.loads((ROOT/'tmp/recovered-sharpen-segment/cgvqm/jobs.json').read_text());assert len(jobs)==16
    ...
    for scene in ('bistro','minecraft'):
        for window in ('central','transition'):
            ...
            effects.append(dict(scene=scene,window=window,disable_sharpen_main=((x[1]-x[0])+(x[3]-x[2]))/2,
                segment_main=((x[2]-x[0])+(x[3]-x[1]))/2,interaction=x[3]-x[2]-x[1]+x[0],both_minus_corrected=x[3]-x[0]))
            for name,path in [
