...
def main():
    p=argparse.ArgumentParser();p.add_argument('--manifest',type=Path,default=ROOT/'tmp/recovered-sharpen-segment/runs.json')
    p.add_argument('--output',type=Path,default=ROOT/'tmp/recovered-sharpen-segment/cgvqm');a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True)
    jobs=[]
    for scene in HISTORIC:
        ...
        for window,start,count in [('central',150,180),('transition',410,30)]:
            ...
            for name in NAMES:
                ...
                reused=name=='Sharpen-Component';dest=a.output/scene/name/window
                output=(ROOT/'tmp/recovered-clipping/cgvqm'/scene/'SignedChroma-YCoCgClamp'/window/'CGVQM-Results.json') if reused else dest/'CGVQM-Results.json'
                fresh=False
                if not output.exists():
                    ...
                    fresh=True
                result=json.loads(output.read_text());expected='O-ET2X-R-SourceIntegrated-SignedChroma-YCoCgClamp' if reused else run['semantic_id']
