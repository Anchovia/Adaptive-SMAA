param(
    [ValidateSet('Capture','Smoke','ScreenBenchmark','GroupCapture','GroupSmoke','GroupScreenBenchmark','Benchmark')][string] $Phase = 'Capture',
    [ValidateSet('bistro','minecraft')][string] $Scene = 'bistro',
    [string] $Receipt = 'tmp/temporal-speed-runs.json'
)
...
$argsForDemo = @("-smaaTemporalSpeed$Phase",$Scene)
