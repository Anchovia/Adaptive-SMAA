"""Same-pass Point/Linear gate: exact controls, spatial proxy and repeated timing."""
import argparse
import csv
import hashlib
import json
...
NATIVE = 'O-T2X-R'
POINT = 'ABL-HistoryPoint-Control-R'
LINEAR = 'ABL-HistoryLinear-R'
SCALAR = 'ABL-ScalarWeight-001-R'
SLINEAR = 'ABL-ScalarHistoryLinear-001-R'
MODES = (NATIVE, POINT, LINEAR, SCALAR, SLINEAR)
...
def read_receipt(path, scene, phase):
    ...
    for token in ('Aggregate: PASS', f'Scene: {scene}', '1920 x 1061',
                  'History filter gate: paired pattern On', 'API:  DirectX11'):
...
def quality(a):
    ...
    names = (*MODES, MASK, SPATIAL, LINEAR+'-Repeat', SLINEAR+'-Repeat')
    assert len(checks) == 2160
    ...
    for i, f in enumerate(expected):
        ...
        for full, selected in ((NATIVE, SCALAR), (LINEAR, SLINEAR)):
            assert np.array_equal(images[selected][mask], images[full][mask]), (i, selected, 'selected')
            assert np.array_equal(images[selected][~mask], spatial[~mask]), (i, selected, 'unselected')
        if i == 0:
            first_seed_differences = {m: int(np.count_nonzero(im != spatial)) for m, im in images.items()}
        counts.append(dict(frame=i, selected_pixels=int(mask.sum()), selected_percent=float(mask.mean()*100),
                           mask_switch_percent=float(np.mean(mask != prev_mask)*100) if prev_mask is not None else None))
        for m, im in images.items():
            ...
            prev[m] = im, y
        prev_ref, prev_mask = ry, mask
        if i % 60 == 59:
            print(f'{a.scene}: {i+1}/240 frames, exact selection semantics PASS', flush=True)
    result = dict(scene=a.scene, classification='Same-pass history filtering engineering gate; spatial proxy, not absolute ghosting or temporal ground truth',
        receipt=record, reference_analysis_sha256=sha(qpath), reference=str(reference),
        pattern_checks=2160, bridge_and_repeat_comparisons=1680, mismatches=0,
        selection_semantics_frames=480, first_frame_differing_channels=first_seed_differences,
    ...
    labels = ('Original Point', 'Full-screen Linear', 'Scalar Point', 'Scalar Linear')
...
def performance(a):
    ...
    comparisons = {}
    for candidate, control in ((POINT, NATIVE), (LINEAR, NATIVE), (LINEAR, POINT),
