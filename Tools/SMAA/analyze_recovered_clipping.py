...
OLD={'bistro':'20260915_144224','minecraft':'20260915_144617'}
...
def manifest(path,mode,scene):
    ...
    for i,name in enumerate(NAMES):
        r=runs[name];assert r['status']=='PASS' and r['signed_chroma']==i&1 and r['ycocg_clamp']==i>>1
        assert hashlib.sha256(r['utility_source'].encode()).hexdigest()==r['utility_sha256'].lower()
        assert f'#define SMAA_RECOVERED_SIGNED_CHROMA {i&1}' in r['utility_source']
        assert f'#define SMAA_RECOVERED_YCOCG_CLAMP {i>>1}' in r['utility_source']
    ...
    normalized={r['utility_source'].replace(f'#define SMAA_RECOVERED_SIGNED_CHROMA {r["signed_chroma"]}',
        '#define SMAA_RECOVERED_SIGNED_CHROMA 0').replace(f'#define SMAA_RECOVERED_YCOCG_CLAMP {r["ycocg_clamp"]}',
        '#define SMAA_RECOVERED_YCOCG_CLAMP 0') for r in runs.values()}
    ...
    for scene in OLD:
        ...
        for window,(lo,hi) in WINDOWS.items():
            ...
            for key in ['mae','psnr','temporal_delta_residual']:
                ...
                effects[key]=dict(signed_main=((x[1]-x[0])+(x[3]-x[2]))/2,
                    domain_main=((x[2]-x[0])+(x[3]-x[1]))/2,interaction=x[3]-x[2]-x[1]+x[0])
